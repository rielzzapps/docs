...
def test_api_document_accesses_list_authenticated_related_same_user():
    """The maximum role across ancestor documents for a given user should be filled."""
    user = factories.UserFactory()
    client = APIClient()
    client.force_login(user)
    ...
    roles = random.sample(["reader", "editor", "administrator"], 3)
    grand_parent_access = factories.UserDocumentAccessFactory(
        document=grand_parent, user=other_user, role=roles[0]
    )
    parent_access = factories.UserDocumentAccessFactory(
        document=parent, user=other_user, role=roles[1]
    )
    document_access = factories.UserDocumentAccessFactory(
        document=document, user=other_user, role=roles[2]
    )
    ...
    for result in content:
        assert (
            result["max_ancestors_role"] == "owner"
            if result["user"]["id"] == str(user.id)
            else "administrator"
        )

    assert {result["id"]: result["abilities"]["set_role_to"] for result in content} == {
        str(grand_parent_access.id): [],
        str(parent_access.id): [],
        str(document_access.id): choices.RoleChoices.values,
        str(user_access.id): [],
    }
...
def test_api_document_accesses_list_authenticated_related_same_team(
    ...
    """The maximum role across ancestor documents for a given team should be filled."""
    user = factories.UserFactory()
    client = APIClient()
    client.force_login(user)
